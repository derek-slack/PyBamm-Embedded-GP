...
class Embedded_GP_Model:
    ...
    def GP_Processing(self, d=False,betas=[]):
        """
        ...
        num_betas = int((len(betas) - 1)/num_functions)
        # betas_list = betas[:-1].reshape(num_functions,num_betas)
        betas_list = [betas[0:2],betas[2:4],betas[4:6],betas[6:8]]
    ...
    def neg_log_likelihood(self,input_dict):
        """
        ...
        if d:
            ...
            # self.betas = betas
            results, sens, neg_vars = self.equation(input_dict, self.discmtx,self.t, d=d)
        else:
            # betas_list = self.GP_Processing(d=d, betas=betas)
            # self.betas = betas
            results = self.equation(input_dict, self.discmtx,self.t, d=d)

        # Calculate neg log likelihood
        n = len(self.data)
        betas = []
        for beta_str in input_dict:
            betas.append(input_dict[str(beta_str)])
        betas.append(self.variance)
        betas_array = jnp.concatenate([jnp.atleast_1d(b) for b in betas])
        # betas = np.array(betas)
        error = self.data - results.flatten()

        ln_variance = self.variance
        # variance = ln_variance
        variance = jax.nn.softplus(ln_variance) + 1e-6
        # dvar = np.exp(ln_variance)/(1+np.exp(ln_variance))
        neg_log_likelihood = 0.5 * jnp.log(2 * np.pi * jnp.exp(ln_variance)) + (error ** 2 / (2 * ln_variance))

        # d_neg_log_likelihood_prior = 30 / 1000
        # if d:
        #     dnll_vec = []
        #     for i, beta_deriv in enumerate(sens):
        #         d_neg_log_likelihood = jnp.sum((error/variance)*beta_deriv) + d_neg_log_likelihood_prior[i]
        ...
        # neg_log_prior = -jnp.log(jax.scipy.stats.multivariate_normal.pdf(betas[:-1], jnp.zeros((len(betas) - 1)), 1000*jnp.eye((len(betas) - 1))))
        #

        # Evaluate prior on all betas except variance (last element)
        beta_params = betas_array[:-1]  # Exclude variance
        n_params = len(beta_params)

        neg_log_prior = -jnp.log(
            jax.scipy.stats.multivariate_normal.pdf(
                beta_params,
                jnp.zeros(n_params),
                1000 * jnp.eye(n_params)
            )
        )
        ...
        # if not d:
        return jnp.sum(neg_log_likelihood) + neg_log_prior
        # if d:
        #     # print(np.array(dnll_vec))
        #     return jnp.sum(neg_log_likelihood) + neg_log_prior, jnp.array(dnll_vec), neg_vars
    ...
    def d_neg_log_likelihood_create(self):
        ...
        def numerical_grad(betas, h=1e-5):
            ...
            grad = np.zeros(len(betas))
            for i in range(len(betas)):
                t_betas = betas.copy()
                t_betas[i] += h*t_betas[i]
                grad[i] = (self.neg_log_likelihood(t_betas,d=True) - self.neg_log_likelihood_i)/(h*t_betas[i])
        ...
        self.d_neg_log_likelihood = jax.grad(self.neg_log_likelihood)


    def neg_log_likelihood_value_and_grad_create(self):
        self.neg_log_likelihood_value_and_grad = jax.value_and_grad(self.neg_log_likelihood)
    ...
    def HMC(self, epsilon, L, current_q, M, Cov_Matrix):
        ...
        U = self.neg_log_likelihood_value_and_grad
        V = self.neg_log_likelihood
        I = self.create_input_dict
        def unpack_grad(grad_dict):
            g_list = list(grad_dict.values())
            g_list.append(0)
        ...
        p = -p
        current_q_inputs = I(current_q)
        q_inputs = I(q)
        try:
            current_U = V(current_q_inputs)
            current_K = sum(current_p @ Cov_Matrix @ current_p.reshape(-1, 1)) / 2
            proposed_U = V(q_inputs)
            proposed_K = sum(p @ Cov_Matrix @ p.reshape(-1, 1)) / 2
        except:
            print(f"U failed at \n q = {current_q}\n p = {current_p}")
            return  current_q, False, U(current_q_inputs)
        ...
        print(f"{accept} q:{q}")
        inputs_final = I(final)
        V_final = self.equation(inputs_final,[],[])
        plt.plot(self.t, V_final, label='GP-Prediction')
        plt.plot(self.t, self.data, label = 'Data')
        plt.xlabel('Time [s]')
        plt.ylabel('Voltage [V]')
        plt.legend()
        plt.show()
        return final, accept, V(I(final))
    ...
    def leapfrog(self, theta, r, grad, epsilon, f, Cov_Matrix):
        ...
        inpts = self.create_input_dict(thetaprime)
        logpprime, gradprime = f(inpts)
    ...
    def find_reasonable_epsilon(self, theta0):
        ...
        Algorithm 4 from original paper epsilon = 1
        """
        # def f_new(input_dict, theta):
        #     # U = self.neg_log_likelihood(input_dict, theta, d=True)
        #     # grad0 = self.d_neg_log_likelihood(theta)
        #     return U, grad0
        self.neg_log_likelihood_value_and_grad_create()
        def f(input_dict):
            logp0, grad0 =  self.neg_log_likelihood_value_and_grad(input_dict)
            grad_values_list = list(grad0.values())
            grad_values_list.append(0)
            grad_values = np.array(grad_values_list)
            return logp0, grad_values
        # f = f_new
        input_dict = self.create_input_dict(theta0)
        logp0, grad0 = f(input_dict)
    ...
    def create_input_dict(self, betas_list):
        input_dict = {}
        ii=0
        for i in range(int(len(betas_list[0:-1])/2)):
            for j in range(2):
                beta_str_in = 'Beta' + str(i) + str(j)
                input_dict[beta_str_in] = betas_list[ii]
                ii+=1
    ...
    def full_sample(self, draws):
        ...
        self.discmtx = []
        self.Cov_Matrix = np.eye(len(self.init_betas))
        self.M = np.diag([1,1,1,1,1,1,1,1,1])
        ...
        samples = np.ones((draws+1,(len(self.init_betas))))
        if len(self.init_betas) > 0:
            samples[0] = self.init_betas

        self.variance = self.init_betas[-1]
    ...
    def full_routine(self,t, tolerance, parameter_init, GP_Function_dict_list,
        batmodel, solver, PSO_options, way3 = 0, init_betas = None):
        """
        Creates the interaction matrixes and compares the models against eachother.  Taken from methodology
        with a singular GP.
        """
        # relats_in = np.array([])
        parameter_set = parameter_init.copy()

        if init_betas is None:
            init_betas=np.zeros(1)
        else:
            self.init_betas = [beta for func in init_betas for beta in func]
        from PSO import ParticleSwarmOptimizer

        # self.init_betas = np.ones(np.array(self.init_betas))
        ...
        killtest_all = []
        damtx = []
        [damtx.append([]) for _ in range(len(GP_Function_dict_list))]
        if m == 1:
            sett = 1
        elif way3:
            sett = 3
        ...
        def create_interaction_matrix(inputs_PSO, ind, damtx=[], relats=[]):
            indvec = np.zeros((inputs_PSO))
            summ = ind
        ...
        def create_beta_inputs_killed(len_mtx, GP_num, killtest):
            betas_function = []
            betas_keys = {}
            for i in range(len_mtx):
                if i != killtest:
        ...
        def init_solve(sim: pybamm.Simulation, t, data):
            ...
            def equation(input_dict):
                ...
                if isinstance(sol, list):
                    for s in sol:
                        V_i_s = s['Voltage [V]'].entries
                        if len(V_i_s) != len(t):
                            V_t = np.zeros(len(t))
                            V_t[0:len(V_i_s)] = V_i_s
                            V_i_s = V_t
                        V.append(V_i_s)
        ...
        def create_bounds(b0, bi, beta_dict):
            ...
            for i, key in enumerate(beta_dict.keys()):
                if i == 0:
                    bounds.update({key: b0})
                else:
                    bounds.update({key: bi})
            return bounds
        ...
        while True:
            ...
            while 1:
                indvec = []
                vecs = []
                ParamClass = ParamUpdate(parameter_set.copy(),new_betas)
                beta_dict = ParamClass._to_input_dict(new_betas)
                bounds = []
                ...
                for i, GP in enumerate(GP_Function_dict_list):
                    ...
                    bounds.append(create_bounds(GP['B0'], (-25, 25), beta_dict_i))
                global_bounds = {}
